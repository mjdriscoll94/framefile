...
final class SharedImportCoordinatorTests: XCTestCase {
    func testPendingShareImportsIntoChosenCategory() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("FrameFileCoordinatorTests-\(UUID().uuidString)", isDirectory: true)
        ...
        let container = try FrameFileContainer.make(inMemory: true)
    ...
    func testPendingShareImportsAsUnsortedWhenCategoryKeyIsEmpty() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("FrameFileUnsortedCoordinatorTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
