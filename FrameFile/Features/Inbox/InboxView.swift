...
struct InboxView: View {
    ...
    private func list(items: [ScreenshotItem]) -> some View {
        List(items) { item in
            inboxListItem(item)
                .listRowBackground(FrameFileTheme.cardBackground)
                .listRowSeparator(.visible)
                .listRowSeparatorTint(FrameFileTheme.cardStroke)
    ...
    private func selectionIndicator(isSelected: Bool) -> some View {
        Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
            ...
            .background(FrameFileTheme.cardBackground, in: Circle())
    ...
    private var activeFilterBar: some View {
        ...
        .overlay(alignment: .bottom) {
            Rectangle()
                .fill(FrameFileTheme.cardStroke)
                .frame(height: 1)
        }
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack {
                ...
                Button(actionTitle, role: actionRole, action: onConfirm)
                    .buttonStyle(.borderedProminent)
                    .tint(actionRole == .destructive ? .red : FrameFileTheme.brandBlue)
            }
        }
        .padding(18)
