...
struct FrameFileExportManifest: Codable, Sendable {
...
struct FrameFileExportPayload: Sendable {
    let metadata: Data
...
protocol DataExporting: Sendable {
    func makePayload(from items: [ScreenshotItem]) throws -> FrameFileExportPayload
}
...
struct ExportService: DataExporting {
    func makePayload(from items: [ScreenshotItem]) throws -> FrameFileExportPayload {
        ...
        let manifest = FrameFileExportManifest(
            formatVersion: 1,
            exportedAt: .now,
