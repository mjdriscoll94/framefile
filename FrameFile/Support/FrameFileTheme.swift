...
struct FrameFileScreenBackground: View {
    var body: some View {
        LinearGradient(
            colors: [FrameFileTheme.secondaryBackground, FrameFileTheme.canvasBottom],
            startPoint: .topLeading,
            endPoint: .bottomTrailing
        )
        .overlay(alignment: .topTrailing) {
            Circle()
                .fill(FrameFileTheme.brandCyan.opacity(0.13))
        ...
        content
            .background(
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .fill(FrameFileTheme.cardBackground)
            )
            .overlay {
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
...
extension View {
    func frameFileCard(cornerRadius: CGFloat = FrameFileTheme.cardCornerRadius) -> some View {
...
struct FrameFileGroupBoxStyle: GroupBoxStyle {
    func makeBody(configuration: Configuration) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 9) {
                Capsule()
                    .fill(FrameFileTheme.brandGradient)
